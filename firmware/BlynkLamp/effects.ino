...
void drawFrame(int pcnt) {
    ...
    for (unsigned char y = HEIGHT - 1; y > 0; y--) {
        for (unsigned char x = 0; x < WIDTH; x++) {
            ...
            if (y < 8) {
                ...
                double hue = rgbToHue(modes[1].rgb.r, modes[1].rgb .g, modes[1].rgb.b);

                CRGB color = CHSV(
                        (hue * 255) + pgm_read_byte(&(hueMask[y][newX])), // H
                        255, // S
    ...
    for (unsigned char x = 0; x < WIDTH; x++) {
        ...
        CRGB color = CHSV(
                (hue * 255) + pgm_read_byte(&(hueMask[0][newX])), // H
                255,           // S
...
void colorRoutine() {
    for (int i = 0; i < NUM_LEDS; i++) {
        leds[i] = CRGB(modes[14].rgb.r, modes[14].rgb.g, modes[14].rgb.b);
    }
}
